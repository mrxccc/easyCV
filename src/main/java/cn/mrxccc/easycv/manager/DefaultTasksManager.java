...
public class DefaultTasksManager implements TasksManager {
     ...
     * id累加
     */
    protected static volatile int task_id_index = 0;
    ...
    @Override
    public synchronized RecordTask createRecorder(String src, String out) throws Exception {
        RecordTask task = null;
        Recorder recorder = null;
        int id = getId();
        log.debug("创建时，当前池数量：" + pool_size + ",空闲数量：" + idle_size + ",工作数量：" + work_size);
        ...
        // 池中总数量未超出,则新建,若超出，不创建
        else if (pool_size < maxSize) {
    ...
    @Override
    public synchronized RecordTask createImageRecorder(String src, String out, int videoCodec) throws IOException, Exception {
        RecordTask task = null;
        Recorder recorder = null;
        int id = getId();
        log.debug("创建时，当前池数量：" + pool_size + ",空闲数量：" + idle_size + ",工作数量：" + work_size);
        ...
        // 池中总数量未超出,则新建,若超出，不创建
            ...
            recorder = new ImageRecord(src, out, 0, 0, avcodec.AV_CODEC_ID_JPEG2000, 25, 25 * 2, 0, avutil.AV_PIX_FMT_NONE);
            task = new RecordTask(id, src, out, recorder);
            task.setPlayurl(playurl);
            saveTaskAndinitRecorder(task);
            pool_size++;// 池中活动数量增加
...
//					System.out.println("归还后，当前池数量："+pool_size+",空闲数量："+idle_size+",工作数量："+work_size);
    ...
    @Override
    public RecordTask getRecorderTask(Integer id) {
        Iterator<RecordTask> itor = workpool.iterator();
        for (; itor.hasNext(); ) {
